...
def load_element_data():
    """Load element data from JSON file with error handling"""
    try:
        # Use the complete elements data
...
def index():
    ...
    return render_template('index_enhanced.html', elements=elements)
...
def search():
    ...
    # Search in compound names and formulas
    results = []
    query_lower = query.lower()
    
    for compound in compounds:
        if (query_lower in compound['name'].lower() or 
            query_lower in compound['formula'].lower()):
...
def api_search():
    """API endpoint for compound search suggestions"""
    query = request.args.get('q', '').strip()
    compounds = load_compounds_data()
...
@app.route('/api/elements/<int:atomic_number>')
def api_element_detail(atomic_number):
    """API endpoint to get specific element"""
    elements = load_element_data()
    element = next((el for el in elements if el['atomic_number'] == atomic_number), None)
